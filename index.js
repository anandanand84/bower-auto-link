...
var LINK_MODULE_COMMAND = 'bower link -f';
var ADD_LINK_COMMAND    = 'bower link ';
...
console.log('Finding bower modules to link in directory '+__dirname);
...
availableProjectsWalker.on("directories", function (root, dirStatsArray, next) {
    ...
    dirStatsArray.forEach(function(stats){
        ...
        if(isBowerModule(moduleDirectory)){
            linkedProjectCount++;
    ...
    availableProjectsForLink.forEach(function(availableProject){
        ...
        var dependents = require(path.join(currentProjectPath,'bower.json')).dependencies;
